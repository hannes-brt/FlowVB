...
class _LatentVariables( HasTraits ):
    ...
    def update_parameters( self, expected_parameters ):
        """
        Update latent variables.
        """

        self.scatter = self._get_scatter( expected_parameters.nws_scale_matrix_inv,
                                     expected_parameters.nws_mean )

        self.log_smm_mixweight = self._update_log_smm_mixweight( expected_parameters.dirichlet )

        self.log_det_precision = self._update_log_det_precision( 
                            expected_parameters.nws_dof,
                            expected_parameters.nws_scale_matrix )

        self.gamma_param_alpha = self._update_gamma_param_alpha( expected_parameters.smm_dof )

        self.gamma_param_beta = self._update_gamma_param_beta( 
                            expected_parameters.smm_dof,
                            expected_parameters.nws_dof,
                            expected_parameters.nws_scale
                             )

        self.latent_resp = self._update_latent_resp( 
                            expected_parameters.smm_dof,
                            expected_parameters.nws_scale,
                            expected_parameters.nws_dof
                             )

        self.latent_scale = self._update_latent_scale()

        self.latent_log_scale = self._update_latent_log_scale()

        self.latent_scaled_resp = self._update_latent_scaled_resp()
